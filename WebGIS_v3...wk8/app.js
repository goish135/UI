...
function c() // present 顏色路段 //
{
	...
	{	
		...
		var decorator = L.polylineDecorator(polyline, {
			patterns: [{
				offset: 0, repeat: 10, symbol: L.Symbol.dash({ pixelSize: 3, pathOptions: { color: color[i], weight: 5, opacity: 0.5 } })
		}]
		}).addTo(map);
...
function draw(color,latlngs)
{
	var polyline = L.polyline(latlngs);
			var decorator = L.polylineDecorator(polyline, {
			patterns: [{
				offset: 0, repeat: 10, symbol: L.Symbol.dash({ pixelSize: 3, pathOptions: { color: color, weight: 5, opacity: 0.5 } })
		}]
		}).addTo(map);	
}

$.getJSON("./data/ValueData.json", function (data){
	console.log(data)
});
