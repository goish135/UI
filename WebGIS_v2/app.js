...
$.get("HomeController.php?start="+start+"&end="+end, function (data) 
{
	...
	if( global_decorators != null || global_decorators != [])
	{
			for (i in global_decorators)
			{
					map.removeLayer(global_decorators[i]);
			}
	}
	...
	var mybox = ["{o|wCm}ceVDXpA]", "cm|wCq}ceVF_@QmAEMA]?KB_@J}AJmAF}@Hw@", "gl|wCyldeV`B\\", "ei|wC{kdeVER?@?B?@?@@B@BBHXd@JR@??@@?@?@?@?@?BC@ABAHK"];
	var mycolor = ['#F00', '#FF0', '#0F0','#00F'];
	var latlngs = [];
	for (i in mycolor)
	{
		latlngs = L.PolylineUtil.decode(mybox[i]);
		var polyline = L.polyline(latlngs);
		
		var decorator = L.polylineDecorator(polyline, {
			patterns: [{ 
				offset: 0, repeat: 10, symbol: L.Symbol.dash({ pixelSize: 3, pathOptions: { color: mycolor[i], weight: 5, opacity: 0.5 } }) 
		}]
		}).addTo(map);
